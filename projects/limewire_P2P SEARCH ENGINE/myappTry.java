...
public class myappTry extends JMenuBar implements Runnable
{
	...
	public myappTry(Frame frame) throws Exception
	{
			...
			jtp.addTab("MONITOR", null, new JLabel("This is Tab Three"), "Files in library");
							...
							if(opt.equals("CONNECT"))
							{
								...
								try
								{
									...
									dirDisplay();
										...
										File libdir = new File("c:\\limewire");
								        String[] s1lib=libdir.list();
								        		 
								       			 	 
								        	
								         libResults = new Object[s1lib.length][titles.length];			
								        if (s1lib == null) 
								        {
								        ...
								        {
								            for (int i=0; i<s1lib.length;i++)                 
								         	{
								         		File f1lib=new File("c:/limewire/"+s1lib[i]);
								         		
								         		libResults[i][0]=s1lib[i];
												libResults[i][1]=f1lib.length();;
									
											}
										}
										libmodel = new DefaultTableModel(libResults,libtitles);
			    						libjt = new JTable(libmodel){
	      											public Dimension getPreferredScrollableViewportSize() 
	      											{
	        											return getPreferredSize();
	      											}
	    								...
	    								Dimension dim = new Dimension(60,1);
	    								libjt.setIntercellSpacing(new Dimension(dim));
									
										libjt.setAutoResizeMode(JTable.AUTO_RESIZE_OFF);
						    			libjt.setRowSelectionAllowed(true);
						
						
										JScrollPane jsplib = new JScrollPane(libjt);
										jsplib.setPreferredSize(new Dimension(790,290));
										...
										jtp.addTab("LIBRARY",null, libPanel, "Monitors the uploading");
